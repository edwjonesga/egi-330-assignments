...
public class BinarySearchTree implements BinaryTree {
    ...
    public int getDepth(int value) {
        ...
        return getDepthRec(root,value,0);
    }

    private int getDepthRec(TreeNode node, int value, int depth){
        if(node == null){
            return -1;
        }
        if(node.val == value){
        ...
        return value < node.val ? getDepthRec(node.left, value, depth+1):getDepthRec(node.right, value, depth+1);
