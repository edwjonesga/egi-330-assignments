This assignment is designed to test your understanding of stacks, queues, and deques. You will be required to implement an ArrayDeque, an ArrayStack, and a LinkedListQueue.
...
```sh
docker build -t stack-de-queue .
```

### Step 5: Once the image is built, run the container using:
#### Linux/Mac:
```sh
docker run -it --rm -v "$(pwd)":/workspace stack-de-queue
```
#### Windows:
```sh
docker run -it --rm -v "%cd%":/workspace stack-de-queue
```
### Step 6: Get most recent assignment code.
Once your workspace is running run the following command to get the most recent code from github.
Select your class then select the current assignment short name: stack-de-queue
```sh
...
5. **refresh-assignment-files.sh:** This script downloads a fresh copy of the assignment files into your workspace to a directory named stack-de-queue.
    ```sh
    refresh-assignment-files.sh
    ```
    You will need to copy the files you want to replace from the stack-de-queue directory.
